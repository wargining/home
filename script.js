...
async function loadChart() {
  try {
    ...
    if (summary.generated_by === "fallback") {
      summaryElement.title = summary.message;
    }
  } catch (error) {
    chartList.innerHTML = `<p class="error-message">데이터를 불러오지 못했습니다. FastAPI 서버가 실행 중인지 확인해 주세요.</p>`;
